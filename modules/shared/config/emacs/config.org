...
*** Basics
These were recommended by Gemini. check if they are useful
#+begin_src emacs-lisp
...
***** COMMENT Custom font
#+begin_src emacs-lisp
  (defface my-org-result '((t (:inherit 'fixed-pitch)))
  "Face used for org-babel results."
  :group 'org-faces)

  ;; (defun my-set-org-result-face ()
  ;;   "Set the font for `org-agenda-mode'."
  ;;   (buffer-face-set 'my-org-result))

  ;; (add-hook 'org-mode-hook 'my-set-org-result-face)
  (defun my/org-fontify-proof-blocks (&optional window)
    (interactive)
      ...
      (font-lock-add-keywords nil
                              `(
                                ("#\\+begin_proof.*?\\(\n\\)"
                                 1 'my-org-result)
                                ("\\(#\\+begin_proof.*$\\)"
                                 1 '(face nil display ,(propertize "Proof." 'face '(:family "TeX Gyre Pagella" :slant italic :height 130 :weight semibold))))
                                ;; ("\\(#\\+end_proof.*\\)$"
                                 ;; 1 '(face nil display ,(propertize (concat (make-string (- (window-width) (* (string-width "⏹") 2)) ?\ ) "⏹") 'face '(:inherit nil))))
                                )))
    (add-hook 'window-size-change-functions #'my/org-fontify-proof-blocks nil t))
  (add-hook 'org-mode-hook #'my/org-fontify-proof-blocks)
#+end_src
