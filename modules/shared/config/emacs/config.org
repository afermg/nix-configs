...
TODO fix: It fails to install in macos for some reason.
...
*** Modeline
Hand-rolled =mode-line-format= instead of doom-modeline. Order: meow state, file, date, time, line/col, buffer %, spacer, EOL style, encoding, major mode, git branch, hostname.

=display-time-mode= is enabled purely as a cheap way to force a modeline refresh every minute so the clock ticks; its own string is suppressed.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package modus-themes
    ...
    ;; "trust this theme" answer. Trust themes up front instead of prompting.
