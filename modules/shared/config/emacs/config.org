...
TAB-to-toggle-subtree lives with =dired-subtree= in the Trial section.
#+BEGIN_SRC emacs-lisp
...
**** COMMENT scpaste
Place an HTML copy of a buffer on the web on a server to which the user has SSH access.
#+begin_src emacs-lisp
  (use-package scpaste
    :defer t)
#+end_src
...
**** sis
OS-native input method switching.
#+begin_src emacs-lisp
  (use-package sis
    :defer t)
#+end_src
...
- beacon-mode: A bit annoying, wasn't that nice anyway
