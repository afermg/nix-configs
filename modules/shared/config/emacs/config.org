...
#+begin_src emacs-lisp
  ...
  (use-package mu4e
    ...
    ;; Ensure rbw vault is unlocked before syncing mail.
    ;; When locked, pop up a term buffer for the user to type the master
    ;; password via pinentry-tty.  The buffer auto-kills on success and
    ;; triggers a fresh mail sync via the process sentinel.
    (defun mu4e--rbw-unlock-popup ()
      "Pop up a terminal running `rbw unlock' and sync mail when done."
    ...
    (defun mu4e--check-rbw-before-update (orig-fn &rest args)
      "If rbw is locked, show unlock popup instead of syncing."
      (if (zerop (call-process "rbw" nil nil nil "unlocked"))
          (apply orig-fn args)
        (mu4e--rbw-unlock-popup)))
