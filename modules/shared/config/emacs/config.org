  ...
  (use-package realgud
    ...
    (defun dispatch-realgud-python-debugger ()
      (let* ((file (shell-quote-argument (file-relative-name (buffer-file-name))))
  	   ...
  	   (minibuffer-history (intern (string-join (list "realgud:" (car prefix) "-minibuffer-history")))))
        (message "%s" (list (buffer-file-name) args minibuffer-history))
        (realgud:run-process (car prefix) (buffer-file-name) args minibuffer-history)))
    
    (defun cb-gud--realgud-command-for-mode (mode)
      (pcase mode
        (`python-ts-mode
         #'dispatch-realgud-python-debugger)))
