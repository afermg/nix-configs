...
#+begin_src emacs-lisp
  ...
  (defun afm/org-open-dwim ()
    "Dispatch C-c C-o: on a heading, defer to `org-open-at-point' so
  subtree-aware openers (Doom's dwim, or any advice on
  org-open-at-point) can act over the whole tree.  Anywhere else, open
  every link in the region or current line via
  `afm/org-open-links-in-region'."
    (interactive)
    (if (org-at-heading-p)
