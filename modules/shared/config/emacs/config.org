...
*** Exports
For now we keep the default org-export being sync.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (setq org-export-async-init-file (expand-file-name "~/.local/share/src/nixos-config/modules/shared/org-export-init.el"))
#+end_src
...
*** COMMENT Org export backup
#+begin_src emacs-lisp
  (defun org-export-backup (BACKEND FILE &rest _args);;(BACKEND FILE &optional ASYNC SUBTREEP VISIBLE-ONLY BODY-ONLY EXT-PLIST POST-PROCESSBACKEND FILE &optional ASYNC SUBTREEP VISIBLE-ONLY BODY-ONLY EXT-PLIST POST-PROCESS)
    "Create timestamped backup of target file before export."
      (when (and (file-exists-p FILE)
  	       (string-suffix-p "tex" FILE))
        (message "Backing up %s" FILE)
        (let* (
  	     (sans-extension (file-name-sans-extension FILE))
  	    (backup-name (format "%s.%s.bak.pdf"
  				 sans-extension
                                  (format-time-string "%Y%m%dT%H%M%S")))
  	    )
  	(copy-file (format "%s.pdf" sans-extension) backup-name))))
  (defun org-export-backup-after (BACKEND FILE &rest _args);;(BACKEND FILE &optional ASYNC SUBTREEP VISIBLE-ONLY BODY-ONLY EXT-PLIST POST-PROCESSBACKEND FILE &optional ASYNC SUBTREEP VISIBLE-ONLY BODY-ONLY EXT-PLIST POST-PROCESS)
    "Create timestamped backup of target file after export. TODO combine into one function"
      (when (and (file-exists-p FILE)
  	       (string-suffix-p "tex" FILE))
        (message "Backing up %s" FILE)
        (let* (
  	     (sans-extension (file-name-sans-extension FILE))
  	    (backup-name (format "%s_latest.pdf"
  				 sans-extension))
  	    )
  	(copy-file (format "%s.pdf" sans-extension) backup-name))))

  (advice-add 'org-export-to-file :before 'org-export-backup)
  (advice-add 'org-export-to-file :after 'org-export-backup-after)
#+end_src
