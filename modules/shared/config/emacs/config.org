...
#+begin_src emacs-lisp 
  ...
  (defvar my/elfeed-search-filter-history nil
    "Stack of previous `elfeed-search-filter' values for `my/elfeed-search-back'.")

  (defun my/elfeed-search--push-history (new-filter)
    "Push the current `elfeed-search-filter' onto the history before changing."
    (when (and (stringp elfeed-search-filter)
               (not (equal elfeed-search-filter new-filter)))
      (push elfeed-search-filter my/elfeed-search-filter-history)))
  (advice-add 'elfeed-search-set-filter :before #'my/elfeed-search--push-history)

  (defun my/elfeed-search-back ()
    "Pop and restore the previous `elfeed-search-filter'."
    (interactive)
    (if (null my/elfeed-search-filter-history)
        (user-error "No previous elfeed search filter")
      (let ((prev (pop my/elfeed-search-filter-history))
            ;; Don't let `elfeed-search-set-filter' re-push current filter.
            (advice-state (advice-member-p #'my/elfeed-search--push-history
                                           'elfeed-search-set-filter)))
        (unwind-protect
            (progn
              (advice-remove 'elfeed-search-set-filter
                             #'my/elfeed-search--push-history)
              (elfeed-search-set-filter prev))
          (when advice-state
            (advice-add 'elfeed-search-set-filter :before
                        #'my/elfeed-search--push-history))))))
